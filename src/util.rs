...
impl HeaderChecker {
    ...
    /// call for the header line generated by each input file
    /// return true if the line should be written
    pub fn check_output(&mut self, lw: &LineWriter, f: &TextFile) -> Result<bool> {
        let header = lw.record();
        let fname = f.fname();
        // FIXME - if is_cdx, make sure is also a valid header
        let cdx = f.has_cdx_header();
        if header.is_empty() {
            Ok(false)
        } else if self.saw_one {
            match self.mode {
                HeaderMode::Match => {
                    if cdx {
                        if self.head.is_empty() {
                            return err!("Header found in {}, but first file had none.", fname);
                        }
                        if header != self.head {
                            return err!(
                                "Header Mismatch. First was {}, File {} has {}",
