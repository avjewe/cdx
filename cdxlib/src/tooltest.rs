...
use crate::{Result,get_reader,err,Error,Infile};
...
fn grab(tag: &[u8], buff: &mut Vec<u8>, line : &mut Vec<u8>, reader : &mut Infile, need_read : &mut bool) -> Result<bool> {
...
impl Test {
	...
	loop {
	    ...
	    else if grab(b"#stdin", &mut self.stdin, &mut line, &mut reader, &mut need_read)? ||
		grab(b"#stdout", &mut self.stdout, &mut line, &mut reader, &mut need_read)? ||
		grab(b"#stderr", &mut self.stderr, &mut line, &mut reader, &mut need_read)? {
	    }
	...
	tmp.push("stdin");
	{
	    let mut w = std::fs::OpenOptions::new().write(true).create(true).open(&tmp.as_os_str())?;
...
impl Config {
    /// new
    pub fn new() -> Self {
	Self {pass: 0 , fail : 0, bindir: "./".to_string()}
    }
    /// set bin dir
